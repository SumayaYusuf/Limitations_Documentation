...
# Restructured to support the generalizability ablation, storing results per synthetic dataset
experiment_data = {
    "generalizability_ablation": {
        # This dictionary will store results for each distinct synthetic dataset.
        # Keys will be dataset tags like 'synthetic_t2dm_data_seed_42'.
        # Each value will be a dictionary containing the original experiment_data structure for that dataset.
    }
}
...
def generate_synthetic_t2dm_data(
    ...
    random_seed=42,  # This parameter is now critical for generating diverse datasets
):
...
# --- Training Function ---
def train_model(
    model, dataloader, criterion, optimizer, epochs, model_idx, act_name, dataset_tag
):
    model.train()
    current_dataset_exp_data = experiment_data["generalizability_ablation"][dataset_tag]
    # Ensure the list for this activation function exists in the current dataset's experiment data
    if act_name not in current_dataset_exp_data["losses"]["train"]:
        current_dataset_exp_data["losses"]["train"][act_name] = []
    ...
    for epoch in range(epochs):
        ...
        current_dataset_exp_data["losses"]["train"][act_name].append(
            {"model_idx": model_idx, "epoch": epoch, "loss": epoch_loss}
        )


# --- Evaluation Function ---
def evaluate_model(model, dataloader, criterion, model_idx, act_name, dataset_tag):
    model.eval()
    total_loss = 0.0
    predictions_list = []
    ground_truth_list = []
    ...
    current_dataset_exp_data = experiment_data["generalizability_ablation"][dataset_tag]
    # Ensure the list for this activation function exists in the current dataset's experiment data
    if act_name not in current_dataset_exp_data["losses"]["val"]:
        current_dataset_exp_data["losses"]["val"][act_name] = []
    ...
    # Store loss for the current activation function and dataset
    current_dataset_exp_data["losses"]["val"][act_name].append(
...
# --- Core Experiment Logic for a Single Dataset ---
def run_activation_tuning_for_single_dataset(
    random_seed, dataset_tag, num_participants_per_stage=200
):
    print(
        f"\n--- Running activation tuning for dataset: {dataset_tag} (Seed: {random_seed}) ---"
    )

    # Initialize the structure for this specific dataset within experiment_data
    experiment_data["generalizability_ablation"][dataset_tag] = {
        "maecmpr_per_stage": {},
        "metrics": {"train": {}, "val": {}},
        "losses": {"train": {}, "val": {}},
        "stage_labels": [],
    ...
    # Create a local reference to the current dataset's data structure for easier access
    current_dataset_exp_data = experiment_data["generalizability_ablation"][dataset_tag]

    # --- Generate data and split once for all tuning runs for THIS dataset ---
    data, stage_labels, num_modalities, num_features_per_modality = (
        generate_synthetic_t2dm_data(
            random_seed=random_seed,
            num_participants_per_stage=num_participants_per_stage,
        )
    )
    total_features = data.shape[1]

    # The `random_split` uses `torch.manual_seed` set in `generate_synthetic_t2dm_data`,
    # ensuring consistent splits for a given `random_seed`.
    ...
    current_dataset_exp_data["stage_labels"] = val_stage_labels_for_maecmpr.tolist()
    ...
    for act_name, activation_fn_instance in activation_functions_to_tune.items():
        print(
            f"\n--- Running experiment with Activation Function: {act_name} for {dataset_tag} ---"
        )
        ...
        # Initialize lists for current activation within current_dataset_exp_data
        current_dataset_exp_data["metrics"]["train"][act_name] = []
        current_dataset_exp_data["metrics"]["val"][act_name] = []
        current_dataset_exp_data["val_predictions"][act_name] = []
        current_dataset_exp_data["val_ground_truth"][act_name] = []
        ...
        for i in range(num_modalities):
            ...
            print(
                f"Modality {i+1} (Predicting from others) with {act_name} for {dataset_tag}): Validation MAE Loss = {val_loss:.4f}"
            )
        ...
        current_dataset_exp_data["metrics"]["val"][act_name].append(
            {"overall_avg_mae_all_models": avg_val_loss_all_models}
        )
        current_dataset_exp_data["overall_avg_mae_all_models"][
            act_name
        ] = avg_val_loss_all_models

        print(
            f"\nAverage Validation MAE across all {num_modalities} cross-modal models with {act_name} for {dataset_tag}: {avg_val_loss_all_models:.4f}"
        )

        # Store predictions and ground truth for this activation function run and dataset
        current_dataset_exp_data["val_predictions"][
            act_name
        ] = val_predictions_per_modality_current_run
        current_dataset_exp_data["val_ground_truth"][
            act_name
        ] = val_ground_truth_per_modality_current_run
        ...
        print(
            f"\n--- Mean Absolute Cross-Modal Prediction Residual (MAECMPR) per Stage for {act_name} for {dataset_tag} ---"
        )
        stage_names = [
            "Healthy",
            "Pre-diabetes",
        ...
        current_dataset_exp_data["maecmpr_per_stage"][act_name] = final_maecmpr_values


# --- Global Experiment Orchestration for Ablation Study ---
def run_ablation_study():
    # Define the random seeds for diverse synthetic data instances (at least three)
    diverse_random_seeds = [42, 123, 789]

    for seed in diverse_random_seeds:
        dataset_tag = f"synthetic_t2dm_data_seed_{seed}"
        run_activation_tuning_for_single_dataset(seed, dataset_tag)

    # Save all experiment data after all datasets have been processed
    np.save(
    ...
    print(
        f"\nAll ablation study experiment data saved to {os.path.join(working_dir, 'experiment_data.npy')}"
    )
