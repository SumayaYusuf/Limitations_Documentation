...
WEIGHT_DECAY_VALUES = [
    ...
    0.001,
    0.01,
]  # Range of weight decay values to test

# --- Experiment data storage ---
# Updated structure to support hyperparameter tuning
experiment_data = {
    "weight_decay_tuning": {}  # This will store results for each weight decay value
}
...
# --- Training Function ---
def train_model(
    model, dataloader, criterion, optimizer, epochs, model_idx, current_run_results_dict
):
    model.train()
    for epoch in range(epochs):
        ...
        current_run_results_dict["losses"]["train"].append(
            {"model_idx": model_idx, "epoch": epoch, "loss": epoch_loss}
        )


# --- Evaluation Function ---
def evaluate_model(model, dataloader, criterion, model_idx, current_run_results_dict):
    model.eval()
    total_loss = 0.0
    predictions_list = []
    ground_truth_list = []
    ...
    with torch.no_grad():
        for i, batch in enumerate(dataloader):
            ...
            targets = batch["targets"].to(
                device
            )  # Corrected from original: targets = batch["inputs"].to(device)
            outputs = model(inputs)
            loss = criterion(outputs, targets)
    ...
    current_run_results_dict["losses"]["val"].append(
...
# --- Main Experiment Logic for a single Weight Decay value ---
def run_single_experiment_for_wd(weight_decay, current_run_results_dict):
    ...
    current_run_results_dict["stage_labels"] = val_stage_labels_for_maecmpr.tolist()
    ...
    for i in range(num_modalities):
        ...
        train_model(
            ...
            epochs=20,
            model_idx=i,
            current_run_results_dict=current_run_results_dict,
        )
    ...
    current_run_results_dict["metrics"]["val"].append(
    ...
    current_run_results_dict["maecmpr_per_stage"] = final_maecmpr_values


def main_tuning_loop():
    print("--- Starting Weight Decay Hyperparameter Tuning ---")
    global experiment_data

    for wd in WEIGHT_DECAY_VALUES:
        print(f"\n======== Running experiment with weight_decay = {wd} ========")
        wd_key = f"wd_{wd}"  # Key for this specific weight decay value

        # Initialize the structure for this weight decay value's results
        experiment_data["weight_decay_tuning"][wd_key] = {
            "maecmpr_per_stage": [],
            "metrics": {"train": [], "val": []},
            "losses": {"train": [], "val": []},
            "stage_labels": [],
        }
        current_run_results_dict = experiment_data["weight_decay_tuning"][wd_key]

        run_single_experiment_for_wd(wd, current_run_results_dict)

        # Print summary for this WD value
        print(f"\n--- Summary Results for weight_decay = {wd} ---")
        maecmpr_results = current_run_results_dict["maecmpr_per_stage"]
        # Assuming metrics/val will have at least one entry after evaluation
        avg_overall_mae = current_run_results_dict["metrics"]["val"][0][
            "overall_avg_mae_all_models"
        ]
        print(f"Overall Avg MAE across all models: {avg_overall_mae:.4f}")
        stage_names = [
            "Healthy",
            "Pre-diabetes",
            "Medication-controlled",
        ...
        for s_idx, maecmpr_val in enumerate(maecmpr_results):
            print(f"Stage {s_idx} ({stage_names[s_idx]}): MAECMPR = {maecmpr_val:.4f}")

    # After all runs, save all experiment data
