...
# Restructured to support hyperparameter tuning for activation functions
experiment_data = {
...
# --- Training Function ---
def train_model(
    model, dataloader, criterion, optimizer, epochs, model_idx, act_name
):  # Added act_name
    model.train()
    # Ensure the list for this activation function exists in experiment_data
    if (
        act_name
        not in experiment_data["activation_function_tuning"]["synthetic_t2dm_data"][
            "losses"
        ]["train"]
    ):
        experiment_data["activation_function_tuning"]["synthetic_t2dm_data"]["losses"][
            "train"
        ][act_name] = []
    ...
    for epoch in range(epochs):
        ...
        # Store loss for the current activation function
        experiment_data["activation_function_tuning"]["synthetic_t2dm_data"]["losses"][
            "train"
        ][act_name].append({"model_idx": model_idx, "epoch": epoch, "loss": epoch_loss})


# --- Evaluation Function ---
def evaluate_model(model, dataloader, criterion, model_idx, act_name):  # Added act_name
    model.eval()
    total_loss = 0.0
    predictions_list = []
    ground_truth_list = []
    ...
    for act_name, activation_fn_instance in activation_functions_to_tune.items():
        ...
        for i in range(num_modalities):
            ...
            val_loss, predictions, ground_truth = evaluate_model(
                model, val_dataloader, criterion, model_idx=i, act_name=act_name
            )
        ...
        print(
            f"\n--- Mean Absolute Cross-Modal Prediction Residual (MAECMPR) per Stage for {act_name} ---"
        )
        stage_names = [
            "Healthy",
            "Pre-diabetes",
